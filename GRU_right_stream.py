...
class SymptomStream(nn.Module):
    def __init__(self, max_posts=500, hidden_dim=512):
        super().__init__()
        
        self.attn=nn.Linear(38,1)
        self.activation=nn.Softmax(dim=-1)
        self.gru = nn.GRU(input_size=38, hidden_size=hidden_dim, num_layers=2, batch_first=True, bidirectional=True)
        
        combined_dim = 38 + hidden_dim * 2
        self.projection = nn.Linear(combined_dim, 256)  # Bidirectional GRU output
        self.relu = nn.ReLU()
    ...
    # Extract symptom tensors from batch and stack into a single tensor
        ...
        # Attention mechanism
        attn_weights = self.activation(self.attn(symp_tensor).squeeze(-1))  # Shape: [batch_size, max_posts]
        weighted_symp = torch.bmm(attn_weights.unsqueeze(1), symp_tensor).squeeze(1)  # Shape: [batch_size, 38]
        #x=symp_tensor.mean(1)
        gru_out, _ = self.gru(symp_tensor)  # Shape: [batch_size, max_posts, hidden_dim * 2]
        gru_pooled = gru_out.mean(dim=1)  # Mean pooling over posts: [batch_size, hidden_dim * 2]
        # Concatenate attention output and GRU pooled output
        combined = torch.cat((weighted_symp, gru_pooled), dim=-1)  # Shape: [batch_size, hidden_dim * 2 + 38]

        # Projection and classification
        projection = self.dropout(self.relu(self.projection(combined)))  # Shape: [batch_size, 256]
        logits = self.clf(projection)  # Shape: [batch_size, num_classes]
