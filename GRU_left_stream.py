...
class PsyEx_wo_symp(nn.Module):
    def __init__(self, model_type, num_heads=2, num_trans_layers=1, max_posts=25, freeze=True, pool_type="first") -> None:
        ...
        self.attn_ff = nn.Linear(self.hidden_dim * 2, 1)  # Adjust for bidirectional GRU
        self.dropout = nn.Dropout(self.post_encoder.config.hidden_dropout_prob)
        self.clf = nn.Linear(self.hidden_dim * 2, len(disease2id))  # Adjust for bidirectional GRU
    ...
    def forward(self, batch, **kwargs):
        ...
        # Process through GRU
        x, _ = self.gru(x)  # Shape: [batch_size, num_posts, hidden_dim * 2]

        # Compute attention scores
        attn_scores = torch.softmax(self.attn_ff(x).squeeze(-1), dim=1)  # Shape: [batch_size, num_posts]

        # Compute weighted features
        attn_embedding = torch.bmm(attn_scores.unsqueeze(1), x).squeeze(1)  # Shape: [batch_size, hidden_dim * 2]

        # Apply dropout
        attn_embedding = self.dropout(attn_embedding)
...
if __name__ == "__main__":
    ...
    model = PsyEx_wo_symp(
        ...
        num_heads=2,  # Transformer heads
        num_trans_layers=1,  # Transformer layers
        max_posts=max_posts,
        freeze=True,
        pool_type="first"
    ).to(device=device)
