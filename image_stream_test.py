...
# classifier.load_state_dict(torch.load('modelImageStream.pth'))
# classifier.eval()
criterion = nn.CrossEntropyLoss() # label smoothing to avoid overconfidence
...
for epoch in range(3):
    ...
    for images_path in os.listdir(base_path):
        ...
        inputs = processor(text=diseases, images=images, return_tensors="pt", padding=True).to(device)
        ...
        predictions.append(id2disease[torch.argmax(logits)])
        true.append(disease)

        loss = criterion(logits, target)
        losses.append(loss.item())

report = classification_report(true, predictions, target_names=id2disease)
print(report)
